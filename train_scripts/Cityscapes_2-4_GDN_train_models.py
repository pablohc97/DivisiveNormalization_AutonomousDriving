...
for i in range(len(seeds)):
    ...
    class JaccardScoreCallback(keras.callbacks.Callback):
        ...
        def on_epoch_end(self, batch, logs=None):
            ...
            if jaccard_value > self.best_iou:
              ...
              self.model.save_weights('./Good_train/2-4_gdn/Train_'+str(i)+'/best_model.h5', overwrite = True)
    ...
    def visualize_image_mask(image_paths, mask_paths, preds, colors, img_height = 96, img_width = 256):
        ...
        plt.savefig('./Good_train/2-4_gdn/Train_'+str(i)+'/Predictions.png')
        #plt.show()
        plt.clf()
    ...
    # Save history
    np.save('./Good_train/2-4_gdn/Train_'+str(i)+'/history_train_'+str(i)+'.npy', history_model1.history)
    np.save('./Good_train/2-4_gdn/Train_'+str(i)+'/ious_train_'+str(i)+'.npy', np.asarray(ious))
    ...
    plt.ylabel('Log10(lr)')
    plt.xlabel('Epoch')
    plt.savefig('./Good_train/2-4_gdn/Train_'+str(i)+'/Lr.png')
    #plt.show()
    plt.clf()
    ...
    plt.ylabel('Accuracy')
    plt.xlabel('Epoch')
    plt.savefig('./Good_train/2-4_gdn/Train_'+str(i)+'/Accuracy.png')
    #plt.show()
    plt.clf()
    ...
    plt.ylabel('Loss')
    plt.xlabel('Epoch')
    plt.savefig('./Good_train/2-4_gdn/Train_'+str(i)+'/Loss.png')
    #plt.show()
    plt.clf()
    ...
    plt.ylabel('IoU')
    plt.xlabel('Epoch')
    plt.savefig('./Good_train/2-4_gdn/Train_'+str(i)+'/Iou_validation.png')
    #plt.show()
    plt.clf()

    # Restore best weights
    model1.load_weights('./Good_train/2-4_gdn/Train_'+str(i)+'/best_model.h5')
